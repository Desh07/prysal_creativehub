...
export default function Home() {
  ...
  useEffect(() => {
    fetch(`/api/content?site=print&t=${Date.now()}`, { cache: 'no-store' })
      .then(res => res.json())
  ...
  return (
    <main className="min-h-screen bg-zinc-200 text-zinc-800 relative">
      ...
      <header className="fixed w-full z-50 top-0 flex flex-col">
        {content.offer?.isActive && (
          <div className="bg-print-gradient text-white text-center py-3 px-4 text-sm font-bold shadow-md overflow-hidden">
            {content.offer.text}
          </div>
        )}

        {/* Navbar (Minimalist) */}
        <nav className="w-full bg-zinc-200/90 backdrop-blur-md border-b border-zinc-300 transition-all duration-300">
          <div className={`max-w-7xl mx-auto px-6 flex items-center justify-between transition-all duration-300 ${isScrolled ? 'h-16' : 'h-24'}`}>
            <div className="flex items-center space-x-3">
              ...
              <div className="font-black text-2xl tracking-tight hidden sm:block text-zinc-800">PRYSAL <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-pink-500">PRINT HUB</span></div>
            </div>
            <div className="hidden md:flex space-x-8 text-sm font-bold">
              {['home', 'services', 'contact'].map(section => (
                <a
                  key={section}
                  href={`#${section}`}
                  className={`relative py-2 transition-colors capitalize ${activeSection === section ? 'text-zinc-800' : 'text-zinc-500 hover:text-zinc-800'}`}
                >
                  {section}
                  {activeSection === section && (
                    <motion.div
                      layoutId="navParticle"
                      className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-print-gradient shadow-[0_0_8px_rgba(24,24,27,0.9)]"
            ...
            <div className="hidden md:flex items-center space-x-4">
              <a href="/design" className="text-xs font-bold text-zinc-500 hover:text-zinc-800 bg-zinc-800/5 px-4 py-2 rounded-full border border-black/10 transition-colors flex items-center space-x-2">
                <Icons.Repeat size={14} />
                <span>Switch to Design Hub</span>
              </a>
              <MagneticButton href={content.social?.whatsapp || '#'} target="_blank" rel="noopener noreferrer" className="hidden md:flex items-center justify-center bg-print-gradient hover:opacity-90 text-white px-8 py-3 rounded-full text-sm font-bold shadow-lg transition-all">
                Contact Us
              </MagneticButton>
            </div>
            ...
            <div className="md:hidden flex items-center space-x-2">
              <a href="/design" className="text-zinc-500 hover:text-zinc-800 p-2 bg-zinc-800/10 rounded-full flex items-center justify-center border border-black/10" aria-label="Switch to Design Hub">
                <Icons.Repeat size={20} />
              </a>
              <button
                className="text-zinc-800 p-2 relative w-12 h-12 flex items-center justify-center"
          ...
          <AnimatePresence>
            {isMobileMenuOpen && (
              <motion.div
                ...
                className="md:hidden bg-zinc-200 border-b border-zinc-300 overflow-hidden"
              >
                <div className="flex flex-col p-6 space-y-6 text-center text-lg font-bold">
                  {['home', 'services', 'contact'].map(section => (
                    <a
                      ...
                      className={`capitalize ${activeSection === section ? 'text-zinc-500' : 'text-zinc-800'}`}
                    >
                      {section}
                    </a>
                  ))}
                  ...
                  <a
                    ...
                    className="bg-print-gradient hover:opacity-90 text-white py-4 rounded-xl shadow-lg mt-2"
                  >
                    Contact Us
                  </a>
                </div>
            ...
            {content.customHero.hasButton && content.customHero.buttonText && (
              ...
              >
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center justify-center space-x-2 bg-print-gradient hover:opacity-90 text-white px-8 py-4 rounded-full font-bold shadow-2xl transition-transform hover:scale-105 group/btn"
                >
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-8 lg:gap-16">
          <div className="flex-1">
            ...
            <motion.div
              ...
              className="mt-12 grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8 border-y border-zinc-300 py-8"
            >
              {content.hero.stats.map((stat, idx) => (
                <div key={idx} className="flex flex-col">
                  <span className="text-4xl font-black"><CountingNumber value={stat.value} /></span>
                  <span className="text-xs font-semibold text-zinc-500 uppercase tracking-widest mt-1">{stat.label}</span>
                </div>
              ))}
            </motion.div>
          </div>
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex space-x-3 z-10">
              {validImages.map((_, idx) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all duration-500 cursor-pointer ${idx === currentImg ? 'w-8 bg-zinc-200' : 'w-2 bg-zinc-200/50 hover:bg-zinc-200/80 hover:scale-110'}`}
                />
              ))}
            </div>
          </motion.div>
      ...
      <section id="services" className="py-16 bg-white px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-20">
            <h2 className="text-4xl md:text-5xl font-black tracking-tight text-zinc-800">Our Expertise</h2>
            <p className="mt-4 text-zinc-500 text-lg">Quality printing for every project.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {content.categories && content.categories.map((category, idx) => (
              <motion.div
                ...
                className="relative bg-zinc-950 rounded-[2.5rem] shadow-sm hover:shadow-2xl transition-shadow duration-300 border border-zinc-800 group overflow-hidden aspect-[4/5] md:aspect-[5/4] min-h-[400px]"
              >
                {/* Background Image */}
                ...
                <div className="absolute inset-0 p-6 md:p-10 flex flex-col justify-end z-10">
                  ...
                  <p className="text-zinc-300 text-sm md:text-lg mb-6 leading-relaxed max-w-md">{category.desc}</p>

                  <ul className="space-y-3">
                    {category.items.map((item, i) => (
                      <li key={i} className="flex items-center space-x-3 text-zinc-400 font-medium">
                        <Icons.CheckCircle2 size={18} className="text-orange-500" />
                        <span>{item}</span>
                      </li>
                    ))}
                  </ul>
                  ...
                  <MagneticButton
                    href={content.social?.whatsapp || "#"}
                    target="_blank"
                    className="mt-8 inline-flex items-center justify-center space-x-2 bg-gradient-to-r from-amber-500 to-orange-600 text-white px-8 py-3 rounded-full font-bold shadow-lg transition-all w-fit group/btn"
                  >
      ...
      <section className="py-8 bg-zinc-50 border-y border-zinc-200 overflow-hidden relative">
        <div className="absolute inset-y-0 left-0 w-48 bg-gradient-to-r from-[#fafafa] to-transparent z-10 pointer-events-none"></div>
        <div className="absolute inset-y-0 right-0 w-48 bg-gradient-to-l from-[#fafafa] to-transparent z-10 pointer-events-none"></div>
                ...
                return (
                  <div key={idx} className="flex items-center space-x-12">
                    <div className="flex items-center space-x-4 group cursor-default">
                      <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center border border-zinc-200 group-hover:border-zinc-400 transition-all duration-300 shadow-sm">
                        <Icon size={20} className="text-zinc-500 group-hover:text-zinc-800 transition-colors" />
                      </div>
                      <span className="text-xl font-bold text-zinc-500 group-hover:text-zinc-800 transition-colors tracking-wide">
                        {item.title}
                      </span>
                    </div>
                    {/* Seamless separator */}
      ...
      <section className="py-12 border-y border-zinc-200 bg-zinc-50 px-6">
      ...
      <section className="py-16 bg-white px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-black tracking-tight text-zinc-800 mb-4">Client Testimonials</h2>
            <div className="flex items-center justify-center space-x-2 text-yellow-400">
              <span className="text-xl font-bold text-zinc-800 mr-2">5/5</span>
              {[1, 2, 3, 4, 5].map(i => <Icons.Star key={i} size={24} fill="currentColor" />)}
              <span className="text-zinc-500 ml-2">on Google Reviews</span>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: "Yathushan Ravichandhran", rating: 5, text: "Great printing services provided by Prysal Print Hub in Matale. Their professional and friendly staff has impressed me. Their prints are of outstanding quality, and they always provide excellent service. Prysal Print Hub is a great option if you're searching for a trustworthy and superior printing solution. One of our logo has designed in next level. Wishing you for the success.", date: "A year ago", initials: "YR" },
              { name: "Harishek RK", rating: 5, text: "Mataleலே best & budget-friendly printing shop தேடினா, இதுக்கு மேல நல்ல இடம் கிடையாது! Highly recommended! Designing & printing எதுவாக இருந்தாலும் fast & professional service தராங்க.", date: "A year ago", initials: "HR" },
              { name: "Nirasha Thilakarathna", rating: 5, text: "photocopy, printout, binding, lamination, banners, flex, business cards, invitations සහ ඕනෑම designing & printing කරන්න හොඳම තැන Prysal Print Hub.", date: "Recent", initials: "NT" }
            ].map((review, idx) => (
              <motion.div
                key={idx}
                ...
                className="bg-zinc-50 rounded-[2rem] p-6 md:p-8 border border-zinc-200 flex flex-col justify-between hover:border-zinc-300 shadow-sm hover:shadow-xl transition-all"
              >
                <div>
                  <div className="flex text-yellow-400 mb-6">
                    {[...Array(review.rating)].map((_, i) => <Icons.Star key={i} size={20} fill="currentColor" />)}
                  </div>
                  <p className="text-lg text-zinc-800 leading-relaxed mb-8">"{review.text}"</p>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-print-gradient rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg">
                    {review.initials}
                  </div>
                  <div>
      ...
      <section id="contact" className="py-20 px-6 overflow-hidden relative border-t border-zinc-200">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12">

          {/* Left: Google Map */}
          <div className="w-full lg:w-1/2 relative h-[350px] lg:h-[450px] rounded-3xl overflow-hidden border border-zinc-200 shadow-lg">
            <iframe
          ...
          <div className="w-full lg:w-1/2 text-center lg:text-left">
            <h2 className="text-4xl md:text-7xl font-black tracking-tighter mb-6 text-zinc-800">Let's Talk.</h2>
            <p className="text-xl text-zinc-500 mb-8">Ready to start your next big project? Reach out to our team.</p>

            <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4 mb-10">
              <a href={`tel:${content.contact.phone}`} className="flex items-center justify-center space-x-3 bg-zinc-50 hover:bg-zinc-100 px-8 py-4 rounded-2xl transition-all shadow-sm hover:shadow-md group border border-zinc-200 hover:border-zinc-300">
            ...
            {/* Social Links */}
            <div className="flex justify-center lg:justify-start items-center space-x-6 border-t border-zinc-300 pt-8">
              {content.social?.instagram && (
                <a href={content.social.instagram} target="_blank" rel="noreferrer" className="w-14 h-14 rounded-full bg-zinc-50 flex items-center justify-center text-zinc-500 border border-zinc-300 hover:bg-pink-600 hover:text-white hover:border-pink-600 transition-all shadow-sm hover:shadow-md hover:-translate-y-1">
                  <Icons.Instagram size={24} />
                </a>
              )}
              {content.social?.facebook && (
                <a href={content.social.facebook} target="_blank" rel="noreferrer" className="w-14 h-14 rounded-full bg-zinc-50 flex items-center justify-center text-zinc-500 border border-zinc-300 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all shadow-sm hover:shadow-md hover:-translate-y-1">
                  <Icons.Facebook size={24} />
                </a>
              )}
              {content.social?.whatsapp && (
                <a href={content.social.whatsapp} target="_blank" rel="noreferrer" className="w-14 h-14 rounded-full bg-zinc-50 flex items-center justify-center text-zinc-500 border border-zinc-300 hover:bg-green-500 hover:text-white hover:border-green-500 transition-all shadow-sm hover:shadow-md hover:-translate-y-1">
      ...
      {/* Footer */}
      <footer className="py-8 text-center text-zinc-600 text-sm border-t border-zinc-200 bg-white">
        ...
        {isScrolled && (
          ...
          >
            <MagneticButton
              ...
              className="w-14 h-14 bg-zinc-800 border border-zinc-700 rounded-full flex items-center justify-center text-white shadow-lg hover:bg-zinc-700 transition-all hover:shadow-xl"
            >
...
const ParticleCanvas = () => {
  ...
  useEffect(() => {
    ...
    // Brand gradient colors (Amber/Orange for Print Hub)
    const colors = ['#f59e0b', '#ea580c', '#f97316'];
  ...
  return <canvas ref={canvasRef} className="fixed inset-0 pointer-events-none z-0 opacity-60 mix-blend-multiply" />;
};
